...
# save model
model.save('my_abs_model.h5')  # creates a HDF5 file 'my_model.h5'
...
model = load_model('my_abs_model.h5')
...
while len(validation_predictions) < len(validation_target):
  ...
  last_x_d = scaler.transform(last_x)
  pred_price = model.predict(last_x_d.reshape(1, n, 1))# 1x1 array -> scalar
  pred_price = scaler.inverse_transform(pred_price)
  
  # update the predictions list
  validation_predictions.append(pred_price)
  p = np.array(validation_predictions)
  p = p.reshape((p.shape[0], -1), order='F')
  # make the new input
...
#checking the root mean square error
rmse = np.sqrt(np.mean(p - validation_target)**2)
rmse
...
plt.plot(p, color = 'red', label = 'Predicted S&P500 Stock Price')
plt.title('S&P500 Stock Price Prediction')
plt.xlabel('Trading Day')
plt.ylabel('S&P500 Stock Price')
plt.legend()
